...
//>>built
define(["exports","../shading/PiUtils.glsl","../../shaderModules/interfaces"],function(c,d,b){c.CloudsParallaxShading=function(a){a.fragment.uniforms.add("anchorPosition","vec3").add("anchorPositionCrossFade","vec3").add("fadeInOutFactor","float").add("cloudsHeight","float").add("rotationMatrixClouds","mat4").add("rotationMatrixCloudsCrossFade","mat4").add("radiusCurvatureCorrectionFactor","float").add("radius","float").add("cameraPosition","vec3").add("totalFadeInOut","float").add("crossFade","int").add("crossFadeAnchorFactor",
"float").add("cloudVariables","vec2");a.include(d.PiUtils);a.fragment.code.add(b.glsl`vec3 intersectWithCloudLayer(vec3 dir, vec3 camPos, vec3 spherePos, float radiusReduction)
{
...
}`);a.fragment.code.add(b.glsl`vec3 correctForPlanetCurvature(vec3 dir)
{
dir.z = dir.z*(1.-radiusCurvatureCorrectionFactor) + radiusCurvatureCorrectionFactor;
return dir;
}`);a.fragment.code.add(b.glsl`vec3 rotateDirectionToAnchorPoint(mat4 rotMat, vec3 inVec)
{
return (rotMat * vec4(inVec, 0.0)).xyz;
}`);a.fragment.code.add(b.glsl`const float SUNSET_TRANSITION_FACTOR = 0.3;
...
}`);a.fragment.code.add(b.glsl`vec4 getCloudData(vec3 rayDir)
{
...
}`);a.fragment.code.add(b.glsl`vec4 renderCloud(vec3 worldRay)
{
...
}`);a.fragment.code.add(b.glsl`vec4 renderCloudCrossFade(vec3 worldRay)
{
vec3 intersectionPoint = intersectWithCloudLayer(normalize(worldRay), cameraPosition, anchorPosition, 0.0);
vec3 worldRayRotated = rotateDirectionToAnchorPoint(rotationMatrixClouds, normalize(intersectionPoint));
vec3 worldRayRotatedCorrected = correctForPlanetCurvature(worldRayRotated);
