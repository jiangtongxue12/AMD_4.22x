...
//>>built
define("exports ../views/3d/webgl-engine/core/shaderLibrary/Slice.glsl ../views/3d/webgl-engine/core/shaderLibrary/output/OutputHighlight.glsl ../views/3d/webgl-engine/core/shaderLibrary/util/RgbaFloatEncoding.glsl ../views/3d/webgl-engine/core/shaderModules/interfaces ../views/3d/webgl-engine/core/shaderModules/ShaderBuilder".split(" "),function(e,k,l,m,b,n){function f(c){const a=new n.ShaderBuilder,g=0===c.output,d=1===c.output,h=4===c.output;a.extensions.add("GL_OES_standard_derivatives");a.include(k.Slice,
c);a.attributes.add("position","vec3");a.attributes.add("color","vec3");a.vertex.uniforms.add("uModelViewMatrix","mat4").add("uProjectionMatrix","mat4").add("uScreenMinMaxSize","vec2").add("uPointScale","vec2").add("uClipMin","vec3").add("uClipMax","vec3");d?(a.vertex.uniforms.add("nearFar","vec2"),a.varyings.add("depth","float")):4!==c.output&&a.varyings.add("vColor","vec3");a.vertex.code.add(b.glsl`
  ...
  `);a.fragment.include(m.RgbaFloatEncoding,c);h&&a.include(l.OutputHighlight);a.fragment.code.add(b.glsl`
    void main(void) {
  ...
  `);return a}const p=Object.freeze({__proto__:null,build:f});e.PointRendererShader=p;e.build=f});